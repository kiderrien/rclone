...
/*
func (f *Fs) connectxrootclient(scr string, ctx context.Context) (fi os.FileInfo, err error){
  url, err := xrdio.Parse(scr)
  if err!= nil{

    return nil, errors.Wrap(err, "could not parse "+ scr)
  }
  ...
  fs := client.FS()
    ...
    return nil, errors.Wrap(err," could not stat" + url.Path )
	}
  return fi, nil
}
